...
app = FastAPI(title="koreaapi", description="많은 양의 한국  api를 보여줍니다 꾸준히 업대이트 중입니다.", version="1.0.0", docs_url="/")
...
@app.get("/weather/{location}", tags=["api"])
...
@app.get("/translation/{source}/{target}/{trsText}", tags=["api"])
def translation(source: str, target: str, trsText: str):
...
@app.get("/pokemon/{pokename}", tags=["api"])
async def pokemon(pokename: str):
    try:
        if len(pokename) == 1:
            return {
                "success": False,
